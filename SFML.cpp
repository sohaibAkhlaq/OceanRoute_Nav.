...
SelectBox fromSelect, toSelect, modeSelect;
...
const float detailsLineHeight = 78.f; // Increased for better spacing
...
int main()
{
    ...
    fromSelect.box = toSelect.box = modeSelect.box = RectangleShape(Vector2f(panelWidth - 40, 56));
    fromSelect.arrow = toSelect.arrow = modeSelect.arrow = Text("V", font, 28);
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == Event::MouseWheelScrolled && event.mouseWheelScroll.wheel == Mouse::VerticalWheel)
            {
                ...
                if (panelOpen && !allPaths.empty() && detailsBox.getGlobalBounds().contains(mouse))
                {
                    int edges = allPaths[currentPathIndex].first.size();
                    float contentHeight = edges * detailsLineHeight + 120;
            ...
            if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left && clickTimer <= 0.f)
            {
                ...
                if (hitFrom && activeDropdown == nullptr)
                {
                    ...
                    toSelect.dropdown.open = modeSelect.dropdown.open = false;
                ...
                if (hitTo && activeDropdown == nullptr)
                {
                    ...
                    fromSelect.dropdown.open = modeSelect.dropdown.open = false;
                ...
                if (hitMode && activeDropdown == nullptr)
                {
                    ...
                    fromSelect.dropdown.open = toSelect.dropdown.open = false;
        ...
        auto updateDrop = [&](Dropdown &dd, bool wantOpen)
        {
            ...
            dd.x = panelX + 20;
            dd.y = (&dd == &fromSelect.dropdown) ? 156 : (&dd == &toSelect.dropdown) ? 256
                                                                                     : 356;
            dd.width = panelWidth - 40;
            dd.items = (&dd == &modeSelect.dropdown) ? modeSelect.dropdown.items : allPorts;
        ...
        if (panelOpen && !fromSelect.selected.empty() && !toSelect.selected.empty())
        {
            ...
            if (needRecalc)
            {
                ...
                if (modeSelect.selected == "All Possible Paths")
                    allPaths = g.allValidPaths(fromSelect.selected, toSelect.selected);
                else
                {
                    auto [edges, nodes] = g.dijkstraPath(fromSelect.selected, toSelect.selected);
                    if (!edges.empty())
        ...
        {
            // FROM / TO / MODE
            fromSelect.box.setPosition(panelX + 20, 100);
            ...
            fromSelect.text.setPosition(panelX + 35, 113);
            window.draw(fromSelect.text);
            fromSelect.arrow.setPosition(panelX + panelWidth - 60, 108);
            window.draw(fromSelect.arrow);

            toSelect.box.setPosition(panelX + 20, 200);
            ...
            toSelect.text.setPosition(panelX + 35, 213);
            window.draw(toSelect.text);
            toSelect.arrow.setPosition(panelX + panelWidth - 60, 208);
            window.draw(toSelect.arrow);

            modeSelect.box.setPosition(panelX + 20, 300);
            ...
            modeSelect.text.setPosition(panelX + 35, 313);
            window.draw(modeSelect.text);
            modeSelect.arrow.setPosition(panelX + panelWidth - 60, 308);
            window.draw(modeSelect.arrow);

            Text fl("From:", font, 24);
            fl.setFillColor(Color::White);
            fl.setPosition(panelX + 20, 60);
            window.draw(fl);
            Text tl("To:", font, 24);
            tl.setFillColor(Color::White);
            tl.setPosition(panelX + 20, 160);
            window.draw(tl);
            Text ml("Mode:", font, 24);
            ml.setFillColor(Color::White);
            ml.setPosition(panelX + 20, 260);
            window.draw(ml);
            ...
            if (modeSelect.selected == "All Possible Paths" && !allPaths.empty())
            {
                ...
                counter.setPosition(panelX + 35, 370);
                window.draw(counter);

                prevBtn.setPosition(panelX + 20, 400);
                nextBtn.setPosition(panelX + panelWidth - 70, 400);
                prevBtn.setFillColor(Color(70, 70, 70));
                nextBtn.setFillColor(Color(70, 70, 70));
                window.draw(prevBtn);
                window.draw(nextBtn);
                prevTxt.setPosition(panelX + 35, 405);
                nextTxt.setPosition(panelX + panelWidth - 55, 405);
                window.draw(prevTxt);
                window.draw(nextTxt);
            }

            // SCROLLABLE ROUTE DETAILS PANEL - NOW PERFECT
            // === SIMPLE ROUTE DETAILS PANEL ===
            if (!allPaths.empty())
            {
                auto &path = allPaths[currentPathIndex];
                int edges = path.first.size();
                float contentHeight = edges * detailsLineHeight + 180;
                bool needsScroll = contentHeight > detailsMaxHeight;

                // PANEL BACKGROUND
                detailsBox = RectangleShape(Vector2f(panelWidth - 40, detailsMaxHeight));
                detailsBox.setPosition(panelX + 20, 480);
                detailsBox.setFillColor(Color(40, 40, 50, 220)); // simple dark panel
                window.draw(detailsBox);

                // Title
                Text title("Route Details", font, 24);
                title.setFillColor(Color::White);
                title.setStyle(Text::Bold);
                title.setPosition(panelX + 35, 455);
                window.draw(title);

                FloatRect view(panelX + 30, 495, panelWidth - 60, detailsMaxHeight - 30);
                float y = 495 - detailsScrollOffset;
                ...
                for (int i = 0; i < edges; ++i)
                {
                    ...
                    if (y + 85 > view.top && y < view.top + view.height)
                    {
                        ...
                        // Leg: From → To
                        Text leg(from->name + " -> " + to->name, font, 18);
                        leg.setFillColor(Color::White);
                        leg.setPosition(panelX + 40, y + 8);
                        window.draw(leg);

                        // Date & Time
                        Text timeInfo(e->date + " | " + e->dep + "-> " + e->arr + " (" + minutesToString(duration) + ")", font, 16);
                ...
                // TOTAL
                Text total("TOTAL: $" + to_string(totalCost) + " | Duration: " + minutesToString(totalMinutes), font, 20);
                total.setFillColor(Color::White);
                total.setStyle(Text::Bold);
                total.setPosition(panelX + 35, y + 10);
                if (y + 40 < view.top + view.height)
                    window.draw(total);

                // Final Arrival
                if (edges > 0)
                {
                    string arrival = "Arrive: " + path.first.back().second->date + " at " + path.first.back().second->arr;
                    Text arrive(arrival, font, 16);
                    arrive.setFillColor(Color(200, 255, 200));
                    arrive.setPosition(panelX + 35, y + 35);
                    if (y + 60 < view.top + view.height)
                        window.draw(arrive);
                }

                // Scrollbar (simplified rectangle)
                // Scrollbar (fixed)
                if (needsScroll)
                {
                    float trackH = detailsMaxHeight - 60;
                    detailsScrollTrack = RectangleShape(Vector2f(10, trackH));
                    detailsScrollTrack.setPosition(panelX + panelWidth - 35, 505);
                    detailsScrollTrack.setFillColor(Color(70, 70, 80, 180));
                    window.draw(detailsScrollTrack);

                    float scrollableHeight = contentHeight - detailsMaxHeight;
                    float ratio = detailsMaxHeight / contentHeight; // thumb height ratio
                    float thumbH = max(40.f, ratio * trackH);       // min thumb height
                    float maxTravel = trackH - thumbH;

                    // Clamp scroll offset
                    float clampedOffset = std::min(std::max(detailsScrollOffset, 0.f), scrollableHeight);

                    float thumbY = 505 + (clampedOffset / scrollableHeight) * maxTravel;
